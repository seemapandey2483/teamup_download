	$(document).ready(function() {
		$('#PolicyFileInfo').jtable({
                          ...
                          var $selectedRowsP = $('#PolicyFileInfo').jtable('selectedRows');
  		   	           ...
  		   	           $( "#dialog-confirm" ).dialog({
  		   	            	...
  		   	            	"Delete": function() {
  		   	            	 $( this ).dialog( "close" );
  		   	            		$.ajax({
	   	            		   url: "company?action=fileAction&actionType=JSON&method=deletePolicy&ids="+idsjoined,
	   	            		  cache: false,
            ...
            actions: {
                listAction: 'company?action=fileAction&actionType=JSON&method=plist',
            },
			fields : {
				Transaction :{
					title :'',
					...
					display : function(filesData){
						...
						$img.click(function(){
							...
							$('#PolicyFileInfo').jtable('openChildTable', $img.closest('tr'),{
								...
								selectOnRowClick : false,
								actions : {
									listAction : 'company?action=fileAction&actionType=JSON&method=pftlist&agentId='
				...
				dl_status : {
					...
					width : '10%',
					edit : true,
					options: { 'C': 'Current', 'A': 'Archived', 'R': 'Resend/Synch', 'D': 'Download'}
				},
				createdDate : {
					title : 'Created',
					width : '15%'
                             ...
                             // $('#SelectedRowList').empty();
                              if ($selectedRows.length > 0) {
                                  ...
                                  $selectedRows.each(function (index,Element) {
                                  	...
                                  	if($(this).data('record').orig_filename==filesData.record.orig_filename && 
                                  ...
                                  idsjoined = ids.join();
                                  $url ="company?action=fileAction&method=dwnlFiles&agentId="+filesData.record.agentId+"&fileName="+filesData.record.orig_filename+"&creationDate="+filesData.record.created_dt+"&trasnIds="+idsjoined;
                              }else {
                            	  $url ="company?action=fileAction&method=dwnlFiles&agentId="+filesData.record.agentId+"&fileName="+filesData.record.orig_filename+"&creationDate="+filesData.record.created_dt;
                              }
	...
	$(document).ready(function() {
		$('#DirectBillFileInfo').jtable({
		   	            	...
		   	            	$( "#dialog-confirm" ).dialog({
		   	            		...
		   	            		buttons: {
		   	            			"Delete": function() {
		   	            				$( this ).dialog( "close" );
		   	            					$.ajax({
		   	            						url: "company?action=fileAction&actionType=JSON&method=deleteDirectBill&ids="+idsjoined,
		   	            						cache: false,
		   	            						success: function(result){
		   	            							$('.jtable-row-selected').removeClass('jtable-row-selected');
            ...
            actions: {
                listAction: 'company?action=fileAction&actionType=JSON&method=dlist',
            },
			fields : {
				Transaction :{
					title :'',
					...
					display : function(filesData){
						...
						$img.click(function(){
							...
							$('#DirectBillFileInfo').jtable('openChildTable', $img.closest('tr'),{
								...
								actions : {
									listAction : 'company?action=fileAction&actionType=JSON&method=dbftlist&agentId='
										+ filesData.record.agentId
										+ '&fileName='
										+ filesData.record.orig_filename
										+ '&creationDate='
