 ...
 */
public class CammGroupNameDisplayBean implements Serializable, DisplayBean
{
	private static final Logger LOGGER = Logger.getLogger(CammGroupNameDisplayBean.class);
	...
	public void init(HttpServletRequest req, HttpServletResponse resp, ServerInfo serverInfo, DatabaseOperation op, FileItem[] items) throws DisplayBeanException
	{
		try
		{
			...
			if (groupName == null || groupName.equals(""))
			{
				...
				groupName = "Migration - " + df.format(new Date(System.currentTimeMillis()));
			}
			groupDesc = req.getParameter("groupDesc");
			if (groupDesc == null)
				groupDesc = "";
			
			// Compile hashtable of agents that are already members of a migration group
			Hashtable hash = new Hashtable();
			AgentInfo[] members = op.getAgentMembersForGroupType("M");
			if (members != null)
			{
			...
			// Compile list of all agents that are not already members of a migration group
			...
			if (agents != null)
			{
				...
				{
					if (agents[i].isLive() && !agents[i].isClientAppRegistered())
					{
						totalAgents++;
