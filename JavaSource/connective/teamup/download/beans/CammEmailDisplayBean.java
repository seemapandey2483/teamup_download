 ...
 */
public class CammEmailDisplayBean implements Serializable, DisplayBean
{
	private static final Logger LOGGER = Logger.getLogger(CammEmailDisplayBean.class);
	...
	public void init(HttpServletRequest req, HttpServletResponse resp, ServerInfo serverInfo, DatabaseOperation op, FileItem[] items) throws DisplayBeanException
	{
		try
		{
			...
			CustomTextFactory factory = new CustomTextFactory(CustomTextFactory.TEXT_AGENT_MIGRATION, 
													CustomTextFactory.TYPE_EMAIL, serverInfo, agent, op);
			factory.setLoginUrl(serverInfo.getRequestUrl(req, "/agency"));
			
			// Get the email subject line
			subject = factory.getEmailSubject();
			if (nextAction.equals(DatabaseFactory.CAMM_2ND_EMAIL))
				subject = "2nd Request: " + subject;
			else if (nextAction.equals(DatabaseFactory.CAMM_3RD_EMAIL))
				subject = "3rd Request: " + subject;
			else if (nextAction.equals(DatabaseFactory.CAMM_FINAL_EMAIL))
