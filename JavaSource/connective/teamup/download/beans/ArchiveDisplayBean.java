 ...
 * files for a specified trading partner.
 */
public class ArchiveDisplayBean implements Serializable, DisplayBean
{
	private static final Logger LOGGER = Logger.getLogger(ArchiveDisplayBean.class);
	...
	private Vector currentFiles = null;
	...
	/**
	 * Constructor for ArchiveDisplayBean.
	 */
	public ArchiveDisplayBean()
	{
		super();
	}
			...
			for (int i=0; i < files.length; i++)
			{
				if (!files[i].getDownloadStatus().equals(DownloadStatus.TEST))
				{
					...
					if (files[i].getDownloadStatus().equals(DownloadStatus.CURRENT))
						currentFiles.addElement(files[i]);
	...
	/**
	 * Returns the number of current (waiting to be downloaded) files.
	 * @return int
	 */
	 ...
	 * @return FileInfo
	 */
	public FileInfo getCurrentFile(int index)
	{
		return (FileInfo) currentFiles.elementAt(index);
	}
