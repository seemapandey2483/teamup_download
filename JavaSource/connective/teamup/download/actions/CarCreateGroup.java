 ...
 */
public class CarCreateGroup implements Action
{
	private static final Logger LOGGER = Logger.getLogger(CarCreateGroup.class);
	/* (non-Javadoc)
	...
	public String perform(HttpServletRequest req, HttpServletResponse resp, ServerInfo serverInfo, DatabaseOperation op, FileItem[] items) throws ActionException
	{
		...
		try
		{
			...
			String addFlag = req.getParameter("addToExisting");
			boolean addToExisting = (addFlag != null && addFlag.equals("Y"));
			
			if (groupName == null || groupName.trim().equals(""))
				return "car.group.name";
			
			AgentGroupInfo group = op.getAgentGroup(groupName);
			if (group != null && !addToExisting)
				nextPage = "car.group.name";
			else
			{
				int groupCount = 1;
				int agtCount = 0;
				...
				boolean agtListIsIDStrings = false;
				...
				if (agentId == null || agentId.length == 0)
				{
					...
					agtList = (ArrayList) req.getSession().getAttribute(ServerInfo.STORE_AGENT_LIST);
					agtListIsIDStrings = true;
				}
				else
				{
				...
				if (group == null)
				{
					// Create the new agent rollout group
					String tempName = groupName;
					if (createMultiGroups && maxAgents > 0 && agtList.size() > maxAgents)
						tempName += " #1";
					firstGroup = tempName;
					group = op.createAgentGroup(tempName, AgentGroupInfo.TYPE_ROLLOUT);
				...
				for (int i=0; i < agtList.size(); i++)
				{
					...
					if (agtListIsIDStrings)
					{
						group.addMember((String) agtList.get(i));
					}
					else
					{
						agent = (AgentInfo) agtList.get(i);
