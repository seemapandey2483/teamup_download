...
public class SaveAmsClaimSettings extends SortVendorSystems {
	
	private static final Logger LOGGER = Logger.getLogger(SaveAmsClaimSettings.class);

	/**
	 * Constructor for SaveAmsSettings.
	 */
	public SaveAmsClaimSettings()
	{
		super();
	}
	...
	public String perform(HttpServletRequest req, HttpServletResponse resp, ServerInfo serverInfo, DatabaseOperation op, FileItem[] items) throws ActionException
	{
		...
		try
		{
			...
			if (newSystem)
			{
				...
				ams = op.createAmsClaimInfo(amsID);
				ams.setCustomSystem(true);
			}
			else
			{
				ams = op.getAmsClaimInfo(amsID);
			...
			if (customSystem)
			{
				...
				ams.setRegClaimControlType(req.getParameter("control_ctype"));
				ams.setRegPolicyControlType(req.getParameter("control_ptype"));
				
				ams.setCompanyDir(req.getParameter("company_dir"));
				ams.setCompanyClaimDir(req.getParameter("company_cdir"));
				ams.setCompanyPolicyDir(req.getParameter("company_pdir"));
				ams.setCompanyChangeDirectory(true);
				ams.setCompanyClaimChangeDirectory(true);
				ams.setCompanyPolicyChangeDirectory(true);
				...
				ams.setCompanyClaimFilename(req.getParameter("company_cfilename"));
				ams.setCompanyPolicyFilename(req.getParameter("company_pfilename"));
				ams.setCompanyChangeFilename(true);
				ams.setCompanyChangeClaimFilename(true);
				ams.setCompanyChangePolicyFilename(true);
				...
				flag = req.getParameter("agent_change_cflag");
				ams.setAgentChangeClaimFilename(flag != null && flag.equals("Y"));
				ams.setFilenameClaimIncrementType(req.getParameter("filename_incr_ctype"));
				
				
				flag = req.getParameter("agent_change_pflag");
				ams.setAgentChangePolicyFilename(flag != null && flag.equals("Y"));
				ams.setFilenamePolicyIncrementType(req.getParameter("filename_incr_ptype"));
			...
			else
			{
				if (ams.isCompanyChangeDirectory())
					ams.setCompanyDir(req.getParameter("company_dir"));
				if (ams.isCompanyClaimChangeDirectory())
					ams.setCompanyClaimDir(req.getParameter("company_cdir"));
				if (ams.isCompanyPolicyChangeDirectory())
					ams.setCompanyPolicyDir(req.getParameter("company_pdir"));
				
				if (ams.isCompanyChangeFilename())
					ams.setCompanyFilename(req.getParameter("company_filename"));
				if (ams.isCompanyChangeClaimFilename())
					ams.setCompanyClaimFilename(req.getParameter("company_cfilename"));
				if (ams.isCompanyChangePolicyFilename())
					ams.setCompanyPolicyFilename(req.getParameter("company_pfilename"));
