...
/**
 * Base functionality to import an ACORD XML file or block of data and store it in the database 
 * for download by the agent.
 * 
 * @author kmccreary
 */
public class ImportAcordXmlBase
{
	private static final Logger LOGGER = Logger.getLogger(ImportAcordXmlBase.class);
	private static final String CLAIM_XML ="C";
	private static final String TRANSCLASS ="TransactionInfo";
	/**
	 * Constructor for ImportAcordXmlBase.
	 */
	public ImportAcordXmlBase()
	{
		super();
	}
											...
											int batch_num, String xmlData, int blockSequence,String xmlHeader) throws Exception
	{
		...
		try {
			...
			String msg = "Error occurred parsing the XML file: "+filename+" during import :" +e.getMessage();
			...
			importHelper.reportImportError(agentid, agentName, participantCode, msg, filename, 
							origFilename, true, status, e,0,DownloadStatus.CLAIM_CURRENT);
		...
		try {
			...
			String claimXMLParentNode = op.getPropertyValue(DatabaseFactory.PROP_CLAIMXML_PARENT_NODE);
			if(StringUtils.isBlank(claimXMLParentNode)){
				...
				if (batch_num > 0) {
					importHelper.reportImportError(agentid, agentName, participantCode, msg, 
							filename, origFilename, interactive, status, null,0,DownloadStatus.CLAIM_CURRENT );
				}
				return status;
			}
			String claimXMLTransNode = op.getPropertyValue(DatabaseFactory.PROP_CLAIMXML_TRANSACTION_NODE);
			if(StringUtils.isBlank(claimXMLTransNode)){
			...
			Element eclaimSvcRs = root.getChild(claimXMLParentNode);
			
			parentMap.put(claimXMLParentNode, eclaimSvcRs);
			
			transactions =  eclaimSvcRs.getChildren(claimXMLTransNode);
			
			if (transactions == null || transactions.size() == 0) {
				// there were no transactions in the xml file
				String msg = "No Claim transactions defined in the XML file:" +filename;
				status.setStatus(FileImportStatus.FAILED_XML_NO_TRANSACTIONS);
				status.setText(msg);
				
				if (batch_num > 0) {
					importHelper.reportImportError(agentid, agentName, participantCode, msg, filename, 
									origFilename, interactive, status, null,0,DownloadStatus.CLAIM_CURRENT);
				}
				return status;
			}
			//ClaimDAO.prepareList();
			List<XmlExport> claimMapList = op.getClaimMapping(CLAIM_XML);
			if(claimMapList== null || claimMapList.size()==0) {
				String msg = "There is no mapping defined for Claims in System.";
				status.setStatus(FileImportStatus.REJECTED_XML_NOT_SUPPORTED);
				status.setText(msg);
				if (batch_num > 0) {
					importHelper.reportImportError(agentid, agentName, participantCode, msg, 
			...
			XmlExport agentMapNode = XMLImportHelper.AgentMappingNode(claimMapList);
			if(XMLImportHelper.checkForMultipleAgent(transactions,agentMapNode)){
				String msg = "File#  " +filename+" contained transactions for multiple agents / contract numbers";
			...
			if (agentid != null && !agentid.equals("")) {
				...
				if (agentInfo != null) {
					try {
						...
						if (agentInfo.getAms() == null || !agentInfo.getAms().isClaimSupported()) {
							String msg = "For AgentId:" + agentid + ",Vendor system: "+agentInfo.getAms().getVendor()+"-"+agentInfo.getAms().getName()+ " does not support Claim XML download";
							status.setStatus(FileImportStatus.REJECTED_XML_NOT_SUPPORTED);
							status.setText(msg);
							if (batch_num > 0) {
								importHelper.reportImportError(agentid, agentName, participantCode, msg, 
						...
						String defaultFilename = agentInfo.getDefaultClaimFilename();
			...
			if (agentInfo == null || !agentActive) {
				...
				if (batch_num > 0) {
					...
					importHelper.reportImportError(agentid, agentName, participantCode, msg, 
							filename, origFilename, interactive, failStatus, null,transactions.size(),DownloadStatus.CLAIM_CURRENT );
				}
			...
			int msgSeq = 0;
			try {
				XmlExport msgSeqtMapNode = XMLImportHelper.MsgSequenceMappingNode(claimMapList);
				if(msgSeqtMapNode== null){
					String msg = "File#  " +filename+" could not determine message sequence number";
					status.setStatus(FileImportStatus.FAILED_XML_DATA_BAD);
					status.setText(msg);
					if (batch_num > 0) {
						importHelper.reportImportError(agentid, agentName, participantCode, msg, 
			...
			if (testFile)
				...
				dlStatus = DownloadStatus.CLAIM_ARCHIVED;
			else
				dlStatus = DownloadStatus.CLAIM_CURRENT;
			...
			if (op.isDuplicateFile(agentid, origFilename, String.valueOf(createdDate)))
			{
				...
				status.setText("AgentId=" + agentInfo.getAgentId() + ", Duplicate file, import failed");
			...
			if (goodFile) {
				...
				for (Element element :transactions) {
					Object value = null;
					TransactionInfo transinfo = fileinfo.createClaimTransaction();
					for(XmlExport cEx:claimMapList) {
					...
					transinfo.getClaimInfo().setSequence(transinfo.getSequence());
					transinfo.setDownloadStatus(dlStatus);
					transinfo.setLastDownloadDate(0);
					transinfo.setNamedInsured(transinfo.getClaimInfo().getInsuredName());
				...
				{
					desc = "AgentId=" + agentInfo.getAgentId() + ",Import Claim file [ " +origFilename+ "] successful";
					if (fileinfo.getDownloadStatus().equals(DownloadStatus.CLAIM_ARCHIVED) ||
								fileinfo.getDownloadStatus().equals(DownloadStatus.DB_ARCHIVED))
						desc += ", file [ " +origFilename+ "] archived";
					if (serverInfo.getCarrierInfo().isDeleteAllImportedFiles())
						desc += "; original file [ " +origFilename+ "] deleted from source directory";
					else if (agentLive)
						desc += "; original file [ " +origFilename+ "] deleted from source directory for live agent";
				}
		...
		catch (Exception e)
		{
			...
			if (fileinfo == null)
			{
				...
				msg = "Error occurred parsing the xml data during import.";
