...
public class ImportPolicyXmlFile extends ImportAcordPolicyXmlBase implements Action {

	private static final Logger LOGGER = Logger.getLogger(ImportPolicyXmlFile.class);
	/**
	 * Default constructor for ImportPolicyXmlFile.
	 */
	public ImportPolicyXmlFile() {
		super();
	}
	...
	public String perform(HttpServletRequest req, HttpServletResponse resp,
		...
		try
		{
			...
			catch (Exception e)
			{
				...
				status.setText("Policy XML File Import error: " + e.getMessage());
			...
			try
			{
				if (fileComplete && chunkNum == 0)
				{
					...
					{
						/*System.out.println("Policy XML File Data received (" + datasize + 
							" bytes) is not the same length as the original file size (" +
							fsize + " bytes)");*/
						
						LOGGER.error("Policy XML File Data received (" + datasize + 
								" bytes) is not the same length as the original file size (" +
								fsize + " bytes)");
					}
				}
		...
		catch (Exception e)
		{
			...
			try
			{
				...
				LOGGER.error("Policy XML: "+iox);
			}
			
			throw new ActionException("Error uploading Policy XML file", e);
		}
